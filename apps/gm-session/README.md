...
| POST | `/api/sheet-builder/<id>/import-mechanic` | Import a library function / toggle / macro onto the sheet (`{name, as?}`; fresh ids; 409 + `suggested` on a name collision; adds missing fields; append-only) |
...
## 0.7.3 — Import macros + toggle functions, pop-up steps, resizable compressed blocks, edit a compressed block in place

- **Import (character sheet → Mechanics → Import…) now takes trigger functions, toggle functions and formula macros.** The campaign library (`editor-scratch/mechanics/<name>.json`) records a `kind` (`function` | `toggle` | `macro`) and the `fields` the automation depends on. Builder **Publish to library**: a selected Function entry publishes a trigger function; an entry set to **Activation: toggle** publishes a toggle function; a selected compressed block with no Function entry publishes a formula macro (its block, plus the fields its `[x]` outputs produce on the source sheet, e.g. `[x]_half` → `STR_half`). Re-publishing an existing name asks first (in-pane; no silent replace). Library files from before 0.7.3 (no `kind`) import as trigger functions.
  - **Collisions never overwrite**: an import whose name is already used on the sheet (entry name for functions/toggles, compressed-block name for macros) gets `409` with a suggested free name; the sheet asks "Import as …" (prefilled, e.g. `rage_2`) or cancel. A macro whose `[x]` outputs are already produced by another macro on the sheet is refused (two macros can't own the same fields). Imported nodes always get fresh ids.
  - **Dependencies**: fields the automation needs are added to the sheet only if missing (existing field definitions are never changed); fields it reads but that the library entry doesn't define are reported ("read as 0"). A toggle's 0/1 field is added (default 0), under its new name if renamed.
  - **Fix**: importing used to replace the compiled sheet's graph with the builder draft and drop its `collapsed` blocks (compiled formula macros disappeared from play until the next Compile). The import now only appends to `build/sheets/<id>.yaml` (nodes, edges, compressed blocks, missing fields).
- **Toggle functions** — a Function entry with **Activation: toggle** (`mode: "toggle"`): the sheet's toggle (Mechanics row or a toggle-mode layout button) flips the 0/1 field named like the entry, then runs the entry's steps with entry output = the new value (1 on / 0 off); flip and writes are saved together. Entries without `mode` (all older data) behave exactly as before (toggle = plain field flip, Trigger runs the graph).
- **Pop-up step** (Automations palette **Pop-up**, `kind: "popup"`: `prompt`, `var`, `vtype` number | text | choice, `default`, `choices` "Label=value, …"). When a trigger or toggle function that contains one runs — GM sheet window or player app sheet — a modal asks for the value (number / text / dropdown, prefilled with the default; "step 2 of 3" when there are several). The answer is an **instance variable for that run only**: the node's output port carries it, field nodes named like the variable read it, and `{var}` in a Send-to-chat label shows it; arithmetic detail shows it as `5 (bonus)`. It is never written to the sheet (writes to that name are dropped) and nothing is synced.
  - Execution is async-capable: all pop-ups of a run are asked **first** (a pop-up that feeds another comes first, otherwise top-to-bottom), then the graph is evaluated once with the answers. So **Cancel / Escape aborts with no side effects** — nothing was rolled, sent or saved (a toggle stays unflipped). Automations without pop-ups run exactly as before (same evaluator; the sync `evaluateNamedFunction` uses pop-up defaults).
  - Works inside compressed **functions** (compressing is layout only). Not inside formula macros: those are always-on computed fields with no "run" to ask during — compile says so.
- **Compressed blocks are resizable** — drag the grip in the bottom-right corner (min 120×56). The size is stored on the block (`w`/`h` in `graph.collapsed`, already written by Compress since 0.6.x) and kept on Save/Compile; blocks saved without a size load at the default 168×72 and saving doesn't add one.
- **Double-click a compressed block to edit it in place** — every other node, block and wire in the pane is hidden; only the block's internals are shown, with wires that cross its edge drawn as read-only **in ⟵ / out ⟶** port stubs. Add/remove/wire/edit nodes as usual (new nodes join the block). Toolbar: **Exit** (discards the edits and re-packages the block unchanged; asks first if anything changed; Esc) and **Save and exit** (keeps the edits, re-packages, saves the builder document). Compress/Expand/Publish are disabled while editing. **Expand** still unpacks a block.
- **Tests** — `tests/popup-step.mjs` (instance variables, cancel, ordering, text/choice, templates, toggle entries, macro rejection, compressed functions, old graphs), `tests/mechanics-import-kinds.py` (kinds, collisions/rename, dependencies, append-only yaml), `tests/upgrade-preservation.py` extended with the 0.7.3 shapes (pop-up/toggle nodes, blocks with/without size, unknown keys: startup + reads byte-identical; import appends only).
