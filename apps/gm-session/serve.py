#!/usr/bin/env python3
"""Offline GM Session: static UI + campaign library / sheet / token API."""
...
class Handler(BaseHTTPRequestHandler):
    ...
    def do_GET(self) -> None:  # noqa: N802
        ...
        if path in ("/", "/index.html"):
            ...
            return
        if path == "/session.js":
            self._send_file(APP_DIR / "session.js")
            return
        if path == "/favicon.ico":
    ...
    def do_PUT(self) -> None:  # noqa: N802
        ...
        if path.startswith("/api/sheet/"):
            ...
            self._send_json(
                200,
                {"ok": True, "actor_id": actor_id, "path": sheet_rel, "bytes": len(text.encode("utf-8"))},
            )
            return
    ...
    def do_POST(self) -> None:  # noqa: N802
        # Alias PUT for clients that prefer POST
        self.do_PUT()
